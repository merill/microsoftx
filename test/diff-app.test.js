...
test('shortcut root and apex are landing modes while unrelated hosts are rejected', () => {
  ...
  assert.equal(resolveShortcutLocation('https://example.com/en-us/entra/example').mode, 'unsupported-host');
  assert.equal(
...
test('Learn URLs convert to the one-letter shortcut and reject other sites', () => {
  assert.equal(
    shortcutUrlForLearnUrl('https://learn.microsoft.com/en-us/entra/identity/example?tabs=portal#step-1'),
    'https://learn.microsoftx.com/en-us/entra/identity/example?tabs=portal#step-1'
  );
